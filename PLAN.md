...
- The sample is built by the nightly job: a `scheduler` container (`make schedule`, `restart: unless-stopped`) runs `coach nightly` at 20:00 in the user's TIMEZONE: sync, then 100 rapid + 100 blitz games at depth 12 on 2 threads. It runs on wake if the Mac was asleep, and a shared age-based lock (`data/.analysis.lock`) makes it skip while another analysis is running. One-off runs: `SINCE=2026-01-01 THREADS=10 HASH_MB=2048 scripts/nightly-analysis.sh`. (A macOS launchd job was dropped: launchd can't read `~/Documents` without granting Full Disk Access.)
...
- Openings tab: shows each repertoire change from `coaching/repertoire.json` on a board. The recommended line and your old line are validated and expanded to positions server-side, with the key move highlighted.
